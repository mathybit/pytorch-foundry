"""Tests for models/example_regressor_dnn/dataset.py."""
...
from models.example_regressor_dnn.dataset import (
    TrainDataset,
    TestDataset,
    InferenceDataset,
    create_datasets,
...
def _write_csv(p: Path, n_features: int = 3, n_rows: int = 5):
    """Write a minimal episode CSV with float targets."""
...
class TestCreateDatasets:
    def _make_cfg(self, tmp_path: Path):
        ...
        csv_dir.mkdir()
        _write_csv(csv_dir / "episodes_reg_v1.csv")
        ...
        cfg = MockConfig()
        cfg.EPISODES_CSV = str(csv_dir / "episodes_reg_v1.csv")
