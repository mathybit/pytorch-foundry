"""
Example Regressor DNN — Datasets

Three dataset classes for the regressor model:
- TrainDataset: reads episode IDs from a train .txt split, yields (X, y_float) via stream
- TestDataset: reads episode IDs from a test/valid .txt split, yields (X, y_float) via iteration
...
class TrainDataset(MultiWorkerStreamDataset):
    """Training dataset for example_regressor_dnn.

    Uses stream-based iteration with infinite cycling.
    """

    def __init__(self, split_txt: Union[Path, str], csv_path: Union[Path, str], shuffle: bool = False, seed: int = None) -> None:
        self.csv_path = Path(csv_path)
        self._feature_cols, self._feat_data, self._tgt_data, self.n_features = load_csv_to_lookup_float(
    ...
    def preprocess(self, point: int):
        ...
        y = torch.tensor(self._tgt_data[point], dtype=torch.float32)
        return X, y


class TestDataset(MultiWorkerIterableDataset):
    """Test/validation dataset for example_regressor_dnn."""
...
def create_datasets(exp_config) -> Dict[str, IterableDataset]:
    ...
    Returns:
        Dict[str, IterableDataset] with keys "train", "test", "valid".
    """
    csv_path = Path(exp_config.EPISODES_CSV)
    split_dir = Path(exp_config.SPLIT_DIR)

    dataset_train = TrainDataset(split_txt=split_dir / "train.txt", csv_path=csv_path, shuffle=True, seed=exp_config.SPLIT_RANDOM_SEED)
